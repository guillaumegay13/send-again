...
alter table if exists send_jobs
...
begin
  if exists (
    select 1
    from pg_constraint
    where conname = 'send_jobs_status_check'
      and conrelid = 'send_jobs'::regclass
  ) then
    alter table send_jobs drop constraint send_jobs_status_check;
  end if;
exception
...
begin
  if not exists (
    ...
    alter table send_jobs
      add constraint send_jobs_status_check
      check (status in ('scheduled','queued','running','completed','failed','cancelled'));
  end if;
exception
  when undefined_table then
...
create table if not exists campaign_runs (
  id uuid primary key,
  campaign_id uuid not null references campaign_workflows(id) on delete cascade,
  workspace_id text not null,
  user_id uuid not null,
  status text not null default 'queued'
...
create table if not exists campaign_run_steps (
  ...
  status text not null default 'pending'
    check (status in ('pending','waiting','processing','completed','failed')),
  due_at timestamptz not null default now(),
  blocking_send_job_id uuid null,
...
create table if not exists scheduled_tasks (
  id uuid primary key,
  kind text not null
    check (kind in ('send_job_dispatch','campaign_step','campaign_wait')),
  workspace_id text not null,
  status text not null default 'queued'
...
create index if not exists idx_campaign_run_steps_run on campaign_run_steps(run_id, status, due_at, created_at);
create index if not exists idx_campaign_run_steps_due on campaign_run_steps(status, due_at, created_at);
create index if not exists idx_scheduled_tasks_due on scheduled_tasks(status, due_at, created_at);
create index if not exists idx_scheduled_tasks_locked on scheduled_tasks(status, locked_at, created_at);
create index if not exists idx_scheduled_tasks_workspace on scheduled_tasks(workspace_id, status, due_at);
